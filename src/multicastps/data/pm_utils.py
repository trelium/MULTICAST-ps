...
def download_pm_data(path, videos = False, id = '', site = 'ch'):
    """
    ...
    :ch: specify which site of data collection to get data for, either 'nyu' or 'ch'
    """
    ...
    url_dict = {
        'ch': {
            ...
            'data_url_base': 'https://pmcp-multicast.bli.uzh.ch/PMCP/api/v04/pcms/downloadInterventionAnalytics/'},
        'nyu': {
            'auth_url': 'https://pmcp-multicast-nyu.bli.uzh.ch/PMCPm-PCMS/api/v00/authentication/login',
            'int_url': 'https://pmcp-multicast-nyu.bli.uzh.ch/PMCPm-PCMS/api/v03/team/basics',          
            'data_url_base': 'https://pmcp-multicast-nyu.bli.uzh.ch/PMCP/api/v04/pcms/downloadInterventionAnalytics/'}
    }

    #get auth info
    auth_cont = { 'username': os.environ.get('PM_USER'), 'password': os.environ.get('PM_PW'), 'twoFactorKey': "0", 'locale': "en-GB" }
    resp = requests.post(url = url_dict[site]['auth_url'], json=auth_cont)
    print(resp)
    auth_tok  = resp.json()
    auth_tok = auth_tok['token'] 
    resp = requests.get(url=url_dict[site]['int_url'], headers={'authorization' : auth_tok})
    objectId = resp.json()
    objectId = objectId['interventionOptions'][0]['objectId']['objectId']
    data_url = url_dict[site]['data_url_base'] + objectId  
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--site",
        help="Specify which site of data collection to get data for, either nyu or ch",
        type=str,
        default='ch'
    )
    
    args = parser.parse_args()
    download_pm_data(args.path, args.videos, args.id, args.site)
